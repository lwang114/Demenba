...
parser.add_argument("--am", type=str, default='htk_mel', choices={'whisper_mel', 'htk_mel', 'whisper_medium'})
parser.add_argument("--classifier", type=str, default='efficientnet')
parser.add_argument("--model_id", type=int, default=7, help='EfficientNet model id')
...
def train(audio_model, train_loader, test_loader, args):
    ...
    for epoch in range(args.n_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            logits = audio_model(audio_input, sizes)
            if args.loss == 'bce':
            ...
            if global_step % 1000 == 0:
                ...
                te_auc_dict, te_confusion = validate(audio_model, test_loader, args, best_loss)
...
def validate(audio_model, val_loader, args, best_loss):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            audio_input = audio_input.to(device, non_blocking=True)
            dementia_labels = dementia_labels.to(device, non_blocking=True)
            sizes = sizes.to(device, non_blocking=True)
...
audio_conf = {'num_mel_bins': 128, 'target_length': audio_length[args.merge_dur], 'freqm': 0, 'timem': 0, 'mixup': 0, 'mode': 'train', 'mean': -4.2677393, 'std': 4.5689974, 'noise': False}

input_dim = feat_dim[am]

# Audio model
if 'whisper' in args.am:
    whisper_model = WhisperReader(args.am, args.download_root, args.layers)
else:
    whisper_model = None
    layers = None

if args.classifier == 'mlp':
    classifier = WhisperMLPClassifier(whisper_model, 1024, 2, input_dim, layers=layers)
else:
    if args.loss == 'bce': 
        classifier = EfficientNetClassifier(whisper_model, layers=layers, model_id=args.model_id)
    else:
        classifier = EfficientNetClassifier(whisper_model, n_class=args.n_class, layers=layers, model_id=args.model_id)
...
if args.mode == 'eval_gold':
    ...
    #te_dataset = FHSDataset(
    #    args.data_dir, args.metadata_path, split=args.test_split, add_silence=args.add_silence, merge_dur=args.merge_dur, n_class=args.n_class,
    #)
    te_dataset = AudiosetDataset(
...
if args.mode == 'train':
    #tr_dataset = FHSDataset(
    #    args.data_dir, args.metadata_path, split='train', add_silence=args.add_silence, num_all_per_class=args.num_train, merge_dur=args.merge_dur, n_class=args.n_class,
    #)
    tr_dataset = AudiosetDataset(
        manifest_dir=args.data_dir,
        metadata_path=args.metadata_path,
        split='train',
        add_silence=args.add_silence,
...
#    tr_dataloader = DataLoader(tr_dataset, collate_fn=tr_dataset.collater, batch_size=args.batch_size, shuffle=True)
    ...
    classifier.load_state_dict(torch.load(exp_dir / 'models/best_audio_model.pth'))
