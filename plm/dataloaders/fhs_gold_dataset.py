...
def make_data_dict(audio_dir, wav_paths, labels):
    '''
    ...
    for p, y in zip(wav_paths, labels):
        ...
        wav_path = str(Path(audio_dir) / f'{wav_id}.wav')
        data_dict[wav_id].append([start, end, [wav_path], y])
    return data_dict
...
class FHSGoldDataset(Dataset):
    def __init__(
        ...
        data_dict = make_data_dict(audio_dir, wav_paths, labels)
    ...
    def __getitem__(self, index):
        ...
        return {'audio_input': x, 'sizes': size, 'dementia_labels': label, 'ids': id_date}
