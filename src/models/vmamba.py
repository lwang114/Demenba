...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        _make_patch_embed = dict(
            ...
            v2=self._make_patch_embed_v2, # add line v3 =
            v3=self._make_patch_embed_v3,
    ...
    def _make_patch_embed(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        ...
        return nn.Sequential(
            ...
            (nn.Identity() if channel_first else Permute(0, 2, 3, 1)),
            (norm_layer(embed_dim) if patch_norm else nn.Identity()), # use this one
        )
    ...
    def _make_patch_embed_v2(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        stride = patch_size // 2 # (128, 1)
        kernel_size = stride + 1 # (128, 1)
        padding = 1
        return nn.Sequential(
    ...
    @staticmethod
    def _make_patch_embed_v3(in_chans=3, embed_dim=96, patch_size=128, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        stride = (1, patch_size)
        kernel_size = (1, patch_size)
        return nn.Sequential(
            nn.Conv2d(in_chans, embed_dim, kernel_size=kernel_size, stride=stride, bias=True),
            (nn.Identity() if channel_first else Permute(0, 2, 3, 1)),
            (norm_layer(embed_dim) if patch_norm else nn.Identity()), # use this one
        )

    # new function make patch embed v3, instead of square input use 128x1
