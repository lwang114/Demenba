...
def train(audio_model, teach_model,train_loader, test_loader, args):  
    ...
    best_p, best_r, best_f1 = -np.inf, -np.inf, -np.inf
    ...
    def _save_progress():
        progress.append([epoch, global_step, best_epoch, best_mAP, best_acc, best_p, best_r, best_f1, best_f1s,
    ...
    if teach_model is not None:
        if not isinstance(teach_model, nn.DataParallel):
    ...
    while epoch < args.n_epochs + 1:
        ...
        class_wise_f1s = [2/(1/(middle_ps[i]) + 1/(middle_rs[i])) for i in range(len(middle_ps))]
        macro_f1 = np.mean(class_wise_f1s)
        ...
        if main_metrics == 'mAP':
            ...
            print("macro_f1: {:.6f}".format(macro_f1))
        ...
        if main_metrics == 'mAP':
            ...
            result[epoch-1, :] = [macro_f1, mAUC, average_precision, average_recall, d_prime(mAUC), loss_meter.avg, valid_loss, cum_acc, cum_mAUC, optimizer.param_groups[0]['lr']]
        ...
        if es_counter >= es_patience and es_patience > 0:
            print('early stopping after training for {:d} epochs with best mAP of {:.6f}'.format(epoch, best_mAP))
            break
