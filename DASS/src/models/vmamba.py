...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.devices = [device for _ in range(len(self.layers))]

    def pick_device(self, idx, ngpu):
        if ngpu == 0:
            return torch.device('cpu')
        return torch.device(f'cuda:{idx % ngpu}')

    def assign_devices(self, ngpu):
        self.devices = []
        for i in range(len(self.layers)):
            device = self.pick_device(i, ngpu)
